...
Each API version folder contains:
 ...
 - a conformance profile, describing the expected behaviour of different API payload properties (generated once the rules file is complete)
...
. Locate the workflow named "Create Feature Branch" (or similar) and click on it.
. Use the "Run workflow" (workflow dispatch) option to specify the API name, TMF ID, and version for your new branch.
+
image::images/part3-create-branch.png[Workflow Dispatch,600,auto,align="center"]
. Trigger the workflow. GitHub Actions will automatically:
.. Create a new branch with the naming convention: `feature/TMF629_Customer_v5.0.x`
.. Set up the default configuration file (`config.yaml`) in the correct directory structure for your API
. Once the workflow completes, your new branch will be available in the repository, ready for development in Codespaces or your local environment.

NOTE: Always use the automated workflow to create new feature branches. This ensures that all required files and structure are present, and that your branch adheres to TMForum naming and organizational standards.
...
=== Change the tooling version used in CodeSpace

To remove the existing oas-tooling container (for example, if it is running with `oas-tooling:latest`) and restart with a specific tooling version, follow these steps.
...
2. Start the tooling with the required version (replace `v8.6.6` with your desired version):
+
[source,bash]
----
...
3. Once the desired tooling version is running successfully, open the "Ports" tab, relaunch the web server/GUI, and perform your testing.
...
You can configure the resource diagram generation options by adding a file ``documentation/diagrams/config.yaml`` with the following options available:

* ``ignorePaths`` is an array of API paths that shouldn't generate diagrams (default is ``/listener/``)
* ``simpleTypes`` is an array of resources that shouldn't show up as an expanded diagram (like Money or Quantity)
* ``includeAllRulesResources`` is boolean, set this to ``true`` to include every resource from the rules file as a top-level diagram (default is false)
* ``autoIncludeSubclassResources`` is boolean (default is true) discriminator subtypes are automatically included as top-level diagrams if their parent resource appears in the OAS paths
* ``includeRulesResourcesMatching`` is an array of values to match against Resource names, which may include regular expressions (e.g "^WsBb")
* ``excludeRulesResourcesMatching`` is an array of values to match against Resource names, which may include regular expressions (e.g "^Product$")
...
Regenerate the conformance profile without losing existing content by creating the file `documentation/conformance/config.yaml` with the line:
...
For each conformance entry, there are two parts, condition and comment.

**Condition** is read by tooling and automation to define how the item should be implemented and tested. The valid values for this field are:

* ``""`` or ``O`` means the item is optional
* ``M`` means the field is mandatory. For object properties, the field is mandatory if the object is present.
* ``M (in response messages)`` is used for mandatory fields that are set by the server and not by the client (such as ``id``). For object properties, the field is mandatory if the object is present in the response.

**Comment** is used to give further information to implementers, but is not parsed or understood by tooling. Use this field to give additional notes about things that cannot be expressed in tooling, such as a note about one field depending on the value of another.
...
Quality assurance is critical for TMForum APIs. These sections provide a framework to catch common issues before they propagate through the generation process.

=== Automated checks

We have a series of automated checks that are continually being expanded. Run the checks on your pull request by applying the "run-checks" label.
The checks are run on every commit or label event on a pull request with the run-checks label applied.

The checks add a comment to your pull request with any areas of concern, so that you can address them. On future runs, the comment gets updated so you may need to scroll up to see it.

To learn more about the checks, or to run them in Codespace or on your local machine, check the `checks/README.md` file in the API repo.
