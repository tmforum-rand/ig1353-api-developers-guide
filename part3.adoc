...
By default, the generator runs in _protective mode_: it updates computed fragments but leaves the main document and `parts/` folder untouched, preserving any manual edits. To enable full regeneration — overwriting all output files and writing the merged conformance YAML back to source — add the following to `documentation/conformance/config.yaml`:
...
These settings are not recommended for source control. The step output will indicate which mode is active (`[PROTECTIVE_MODE]` or `[OPT_IN_GENERATION]`). `optInGeneration` can also be placed in `documentation/config.yaml` to apply to all doc tools at once.
