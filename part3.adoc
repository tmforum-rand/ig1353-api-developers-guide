...
By default, the generator runs in _protective mode_: it updates the automatically-templated sections, but leaves the main document and `parts/` folder untouched, preserving any manual edits. To enable regeneration of the input assets, recreating the parts files and merging existing conformance.yaml file content with a freshly-generated structure — add the following to `documentation/conformance/config.yaml`:
...
updateConformanceYaml — default false Controls whether the merged conformance YAML is written back to disk. On the very first (initial) generation the file is always written. On subsequent runs, the file is only rewritten when this is true; otherwise the generator leaves the existing YAML untouched and instead reports a diff summary of what would change. Set it to true to enable auto-update of 
config.yaml.

processConformanceParts — default false Controls how the parts/ folder is handled. When true, the tooling will overwrite stale files, create missing files, and clean up orphaned files. When false, parts processing is skipped and the tool only emits out-of-sync diagnostics telling you to set processConformanceParts: true.

optInGeneration — default false (protective mode) Master switch for "opt-in generation" mode. When false (the default), the tool runs protectively: it only refreshes generated content and metadata, without overwriting existing target template files or the /parts and main document.

Do not store this file in source control.  The build output will indicate which mode is active (`[PROTECTIVE_MODE]` or `[OPT_IN_GENERATION]`). `optInGeneration` can also be placed in `documentation/config.yaml` to apply to all doc tools at once.
