...
|DCS |Domain Context Specialization |TMForum's approach to creating specialized versions of APIs for specific domains
...
1. **OAS Specification Generation**: The OpenAPI Specification (OAS) file is automatically generated from the API rules file and JSON schema definitions
...
3. **Conformance Profile Generation**: Test specifications are generated to define what constitutes a compliant implementation
4. **CTK Generation**: The Conformance Test Kit (CTK) is built to provide executable tests for API implementations
5. **RI/Sample Code Generation**: Reference Implementation (RI) code samples are produced to demonstrate proper API usage
6. **The Publication Process**: Final artifacts are packaged and published to the TMForum catalog

=== Understanding the Dependencies

Each step depends on the previous ones, creating a pipeline where:

* Changes to the rules file trigger OAS regeneration
* Schema modifications affect both OAS and diagram generation
* All artifacts must be regenerated when core definitions change
...
== API Rules File: Basic API Properties
...
|string
|No
|API version number following [SemVer](https://semver.org/) format: `<major>.<minor>.<patch>`.
...
<4> Concise description, appears in `info.description`.
...
api:
  ...
  version:     5.0.0                                        <7>
----
<1> The *version of the rules file format itself* - not of the API under construction. It is used by the OAS generator to warn of any outdated syntax, and will change very infrequently.
<2> The name of your API. It appears in the OAS file in the `info.title` attribute.
<3> A short name for your API, used in contexts where a brief identifier is needed. It should be a concise version of the full API name, typically without spaces or special characters.
<4> A description of your API. It appears in the OAS file in the `info.description` attribute, which supports CommonMark Markdown. Try to keep this description fairly concise, avoid using formatting tags (as the text may appear in many formats: HTML, PDF and OAS rendering). Avoid use of special non-ASCII characters (such as copyright, trademark symbols) as these can also trip-up later parsing.
<5> The identity allocated by TM Forum to this API - typically in the form "TMFxxx" - where "xxx" is a three digit number. It has no direct translation into the OAS file, but will be used by the tooling in generation.
...
<7> This is the version of your TM Forum API. It follows the SemVer (https://semver.org/) versioning format: *<major>.<minor>.<patch>*. This value appears in the OAS file as attribute: `info.version`.
