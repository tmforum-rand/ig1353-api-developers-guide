...
* ``M`` means the field is mandatory
* ``M (in response messages)`` is used for mandatory fields that are set by the server and not by the client (such as ``id``)

**Comment** is used to give further information to implementers, but is not parsed or understood by tooling.
