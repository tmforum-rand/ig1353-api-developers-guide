...
processConformanceParts — default false Controls how the parts/ folder is handled. When true, the file-copier runs all "consistent mode" operations: migration detection, overwriting stale files, creating missing files, and cleaning up orphaned files. When false, parts processing is skipped and the tool only emits out-of-sync diagnostics telling you to set processConformanceParts: true. Note: it is ignored entirely when removeSectionsForOptionalOperations is true (legacy selective-write mode).
