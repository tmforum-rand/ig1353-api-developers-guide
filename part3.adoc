...
The API rules file implicitly makes references to a number of JSON-Schema files. Each resource named under the `resources:` section above will cause the OAS generator to search in the schema repository (`OAS_Open_API_And_Data_Model/tree/<YourBranch>/schemas`) for a JSON-Schema file fitting the format: `<MyResource>.schema.json`. In parsing _that_ JSON-Schema file, it will follow any `$ref` references to other JSON-Schema files, each defining a JSON object type, until it has achieved a transitive-closure of all dependent schemas. This dependency graph will then form the `components.schemas` section of the resultant OAS file, so that all references can be resolved within the API specification.

*Schema Repository Structure*

The schema files defining the JSON object types that make up the data structures that build up the API's resources are placed in the "*schemas*" directory structure located in the schema repository.

Within that structure, schemas of TMF defined classes are placed in the "*Tmf*" subdirectory and further in the subdirectory named according to the domain *<Domain>* to which the class belongs.

Following the above pattern for schema files for resources, the schema file defininig the JSON object type *<ObjectType>* is named according to the pattern `<ObjectType>.schema.json`.

A schema file defining for an *<ObjectType>* an event *<Event>* is named `<ObjectType><Event>.schema.json`, and its payload is defined by a file `<ObjectType><Event>Payload.schema.json`. Both these kinds of files are placed in the "*Event*" subdirectory.

Schema files for object types of a given *<DCS>* (Domain Context Specialization) are located in subdirectories named "*DCS*", with further a subdirectory for a given DCS, and the analogue sub-structure as for non-DCS TMF classes.
